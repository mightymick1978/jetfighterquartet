...
export function renderCpuCard(cardData, isFaceUp) {
  cpuCardSlot.innerHTML = createCardHTML(cardData, 'cpu', !isFaceUp, null);
}
...
// Hebt das gewählte Stat hervor
export function highlightStat(statKey, chooser, playerValue, cpuValue) {
  // Spielerkarte
  const playerStats = playerCardSlot.querySelectorAll('.card ul li');
  playerStats.forEach(li => {
    if (li.dataset.statKey === statKey) {
      li.classList.add('highlight-win');
    }
  });
  // CPU-Karte
  const cpuStats = cpuCardSlot.querySelectorAll('.card ul li');
  cpuStats.forEach(li => {
    if (li.dataset.statKey === statKey) {
      li.classList.add('highlight-win');
    }

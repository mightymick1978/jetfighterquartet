...
function updateUI() {
  ...
  if (playerCards.length === 0) {
    ...
    playSound('win'); // Verlierer-Sound
    return;
  }
  if (computerCards.length === 0) {
    showMessage("You won the game!");
    playSound('win'); // Gewinner-Sound
    return;
  }
...
function addCategoryClickListeners(cardElement, card) {
  ...
  paragraphs.forEach(p => {
    ...
    p.addEventListener('click', () => {
      ...
      playSound('flip'); // Sound beim Auswählen einer Kategorie
...
function playRound(playerCard, category) {
  ...
  updateUI(); // Karten mit Hervorhebung und Gegnerkarte aufgedeckt
...
function cpuTurn() {
  ...
  selectedCategory = category;
  roundInProgress = true;
  playSound('flip'); // Sound beim CPU-Zug
